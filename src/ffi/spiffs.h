...
#define SPIFFS_BASE_ADDR 0x200000
#define SPIFFS_LOG_PAGE_SIZE 0xff
...
typedef i16 spiffs_file;
// spiffs file descriptor flags
typedef u16 spiffs_flags;
// spiffs file mode
typedef u16 spiffs_mode;
// object type
typedef u8 spiffs_obj_type;
...
typedef i32 (*spiffs_read)(struct spiffs_t* fs, u32 addr, u32 size, u8* dst);
/* spi write call function type */
typedef i32 (*spiffs_write)(struct spiffs_t* fs, u32 addr, u32 size, u8* src);
/* spi erase call function type */
typedef i32 (*spiffs_erase)(struct spiffs_t* fs, u32 addr, u32 size);

#else // SPIFFS_HAL_CALLBACK_EXTRA

/* spi read call function type */
typedef i32 (*spiffs_read)(u32 addr, u32 size, u8* dst);
/* spi write call function type */
typedef i32 (*spiffs_write)(u32 addr, u32 size, u8* src);
/* spi erase call function type */
typedef i32 (*spiffs_erase)(u32 addr, u32 size);
 ...
 */
i32 SPIFFS_probe_fs(spiffs_config *config);
 ...
 */
i32 SPIFFS_mount(spiffs *fs, spiffs_config *config, u8* work,
    u8* fd_space, u32 fd_space_size,
    void *cache, u32 cache_size,
    spiffs_check_callback check_cb_f);
 ...
 */
i32 SPIFFS_creat(spiffs *fs, const char* path, spiffs_mode mode);
 ...
 */
i32 SPIFFS_read(spiffs *fs, spiffs_file fh, void *buf, i32 len);
 ...
 */
i32 SPIFFS_write(spiffs *fs, spiffs_file fh, void *buf, i32 len);
 ...
 */
i32 SPIFFS_remove(spiffs *fs, const char* path);
 ...
 */
i32 SPIFFS_fremove(spiffs *fs, spiffs_file fh);
 ...
 */
i32 SPIFFS_stat(spiffs *fs, const char* path, spiffs_stat *s);
 ...
 * @param s             the stat struct to populate
 */
i32 SPIFFS_fstat(spiffs *fs, spiffs_file fh, spiffs_stat *s);
 ...
 */
i32 SPIFFS_fflush(spiffs *fs, spiffs_file fh);
 ...
 */
i32 SPIFFS_close(spiffs *fs, spiffs_file fh);
 ...
 */
i32 SPIFFS_rename(spiffs *fs, const char* old, const char* newPath);

/**
 * Returns last error of last file operation.
 * @param fs            the file system struct
 */
i32 SPIFFS_errno(spiffs *fs);
 ...
 */
i32 SPIFFS_closedir(spiffs_DIR *d);
 ...
 */
i32 SPIFFS_info(spiffs *fs, u32 *total, u32 *used);
 ...
 * @param fs            the file system struct
 */
i32 SPIFFS_format(spiffs *fs);

/**
 * Returns nonzero if spiffs is mounted, or zero if unmounted.
 * @param fs            the file system struct
 */
u8 SPIFFS_mounted(spiffs *fs);
 ...
 */
i32 SPIFFS_gc_quick(spiffs *fs, u16 max_free_pages);
 ...
 */
i32 SPIFFS_gc(spiffs *fs, u32 size);
 ...
 */
i32 SPIFFS_eof(spiffs *fs, spiffs_file fh);
 ...
 * @param fh            the filehandle of the file to check
 */
i32 SPIFFS_tell(spiffs *fs, spiffs_file fh);
 ...
 */
i32 SPIFFS_set_file_callback_func(spiffs *fs, spiffs_file_callback cb_func);
 ...
 */
i32 SPIFFS_ix_map(spiffs *fs, spiffs_file fh, spiffs_ix_map *map,
    u32 offset, u32 len, spiffs_page_ix *map_buf);
 ...
 */
i32 SPIFFS_ix_unmap(spiffs *fs, spiffs_file fh);
 ...
 */
i32 SPIFFS_ix_remap(spiffs *fs, spiffs_file fh, u32 offs);
 ...
 */
i32 SPIFFS_bytes_to_ix_map_entries(spiffs *fs, u32 bytes);
 ...
 */
i32 SPIFFS_ix_map_entries_to_bytes(spiffs *fs, u32 map_page_ix_entries);
