...
struct RTC_REGS {
  u32 volatile CTRL0;        // 0x00
  u32 volatile COUNTER_ALARM;    // 0x04
  u32 volatile RESET_REASON0;    // 0x08       //FIXME: need better name
  u32 volatile _unknownc;        // 0x0c
  u32 volatile _unknown10;       // 0x10
  u32 volatile RESET_REASON1;    // 0x14       //FIXME: need better name
  u32 volatile RESET_REASON2;    // 0x18       //FIXME: need better name
  u32 volatile COUNTER;          // 0x1c
  u32 volatile INT_SET;          // 0x20
  u32 volatile INT_CLEAR;        // 0x24
  u32 volatile INT_ENABLE;       // 0x28
  u32 volatile _unknown2c;       // 0x2c
  u32 volatile SCRATCH[4];       // 0x30 - 3c
  u32 volatile _unknown40;       // 0x40
  u32 volatile _unknown44;       // 0x44
  u32 volatile _unknown48;       // 0x48
  u32 volatile _unknown4c[7];    // 0x4c - 0x64
  u32 volatile GPIO_OUT;         // 0x68
  u32 volatile _unknown6c[2];    // 0x6c - 0x70
  u32 volatile GPIO_ENABLE;      // 0x74
  u32 volatile _unknown80[5];    // 0x78 - 0x88
  u32 volatile GPIO_IN;          // 0x8c
  u32 volatile GPIO_CONF;        // 0x90
  u32 volatile GPIO_CFG[6];      // 0x94 - 0xa8
};
