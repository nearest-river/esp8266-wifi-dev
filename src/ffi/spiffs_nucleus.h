...
#define SPIFFS_UNDEFINED_LEN            (u32)(-1)
...
#define spiffs_get_cache_page(fs, c, ix) \
  ((u8* )(&((c)->cpages[(ix) * SPIFFS_CACHE_PAGE_SIZE(fs)])) + sizeof(spiffs_cache_page))
...
#endif
{
  ...
  // alignment
  u8 _align[4 - ((sizeof(spiffs_page_header)&3)==0 ? 4 : (sizeof(spiffs_page_header)&3))];
  // size of object
  u32 size;
  // type of object
...
typedef i32 (*spiffs_visitor_f)(spiffs* fs, spiffs_obj_id id, spiffs_block_ix bix, int ix_entry,
...
i32 spiffs_phys_count_free_blocks(
    spiffs* fs);

i32 spiffs_obj_lu_find_entry_visitor(
    spiffs* fs,
...
i32 spiffs_obj_lu_scan(
    spiffs* fs);

i32 spiffs_obj_lu_find_free_obj_id(
    spiffs* fs,
    spiffs_obj_id *obj_id,
    const u8* conflicting_name);

i32 spiffs_obj_lu_find_free(
    spiffs* fs,
    spiffs_block_ix starting_block,
    int starting_lu_entry,
    spiffs_block_ix* block_ix,
...
i32 spiffs_obj_lu_find_id_and_span(
    spiffs* fs,
    spiffs_obj_id obj_id,
    spiffs_span_ix spix,
...
i32 spiffs_obj_lu_find_id_and_span_by_phdr(
    spiffs* fs,
    spiffs_obj_id obj_id,
    spiffs_span_ix spix,
    spiffs_page_ix exclusion_pix,
...
i32 spiffs_page_delete(
    spiffs* fs,
    spiffs_page_ix pix);

// ---------------

i32 spiffs_object_create(
    spiffs* fs,
    spiffs_obj_id obj_id,
    const u8 name[SPIFFS_OBJ_NAME_LEN],
    spiffs_obj_type type,
    spiffs_page_ix* objix_hdr_pix);

i32 spiffs_object_update_index_hdr(
    spiffs* fs,
    spiffs_fd* fd,
    spiffs_obj_id obj_id,
    spiffs_page_ix objix_hdr_pix,
    u8* new_objix_hdr_data,
    const u8 name[SPIFFS_OBJ_NAME_LEN],
    u32 size,
...
i32 spiffs_object_open_by_page(
    spiffs* fs,
    spiffs_page_ix pix,
    spiffs_fd* f,
    spiffs_flags flags,
...
i32 spiffs_object_append(
    spiffs_fd* fd,
    u32 offset,
    u8* data,
    u32 len);

i32 spiffs_object_modify(
    spiffs_fd* fd,
    u32 offset,
    u8* data,
    u32 len);

i32 spiffs_object_read(
    spiffs_fd* fd,
    u32 offset,
    u32 len,
    u8* dst);

i32 spiffs_object_truncate(
    spiffs_fd* fd,
    u32 new_len,
    u8 remove_object);

i32 spiffs_object_find_object_index_header_by_name(
    spiffs* fs,
    const u8 name[SPIFFS_OBJ_NAME_LEN],
    spiffs_page_ix* pix);
...
i32 spiffs_gc_erase_page_stats(
    spiffs* fs,
    spiffs_block_ix bix);

i32 spiffs_gc_find_candidate(
    spiffs* fs,
...
i32 spiffs_gc_quick(
    spiffs* fs, u16 max_free_pages);

// ---------------

i32 spiffs_fd_find_new(
    spiffs* fs,
    spiffs_fd** fd,
    const char* name);

i32 spiffs_fd_return(
    spiffs* fs,
    spiffs_file f);
...
i32 spiffs_lookup_consistency_check(
    spiffs* fs,
    u8 check_all_objects);

i32 spiffs_page_consistency_check(
    spiffs* fs);

i32 spiffs_object_index_consistency_check(
    spiffs* fs);
